...
@Component({
  ...
  styleUrls: ['./segundo-chat.component.scss'],
})
export class SegundoChatComponent  implements OnInit, OnDestroy {
  ...
  {
    const observable = this.chatService.traerChats('SegundoChat');
  ...
  volver() : void
  {
    this.chatsEnviados.forEach(chat => {
      this.chatService.agregarMensaje(chat,'SegundoChat');
    });
